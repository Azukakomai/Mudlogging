...
In petroleum reserve certification, commercial asset transactions, and regulatory filings governed by the \citet{ref3} Petroleum Resources Management System (PRMS), reserve auditors strictly mandate that all petrophysical calculations establishing net pay thickness must be mathematically traceable, physics-based, and auditable \citep{ref2, ref3}. Opaque, "black-box" machine learning predictions cannot be independently audited or defended under SPE PRMS standards, creating an unacceptable regulatory risk for asset operators.
...
\subsection{Architecture: Declarative Parameter Dependency Graph}
...
\subsection{Interface and Visualization: Multi-Track Dashboard}
...
\begin{enumerate}
    ...
    \item \textbf{Dynamic 25\% Column Normalization Rule:} To ease users analytical needs, some input columns goes through normalization so only the irregular data are shown, and none of the common uneeded data are shown in the columns.
...
\subsection{Deployment: Zero-Installation Serverless Edge Architecture}
